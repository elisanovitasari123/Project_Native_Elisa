...
if (isset($_POST['id_villa'])) {
    $id_villa = mysqli_real_escape_string($db, $_POST['id_villa']);

    // Mempersiapkan query SQL untuk menghapus data berdasarkan id_pelanggan_bulanan
    $sql = "DELETE FROM villa WHERE id_villa = '$id_villa'";
    ...
    echo json_encode(array('status' => 'error', 'message' => 'ID villa tidak dikirim'));
}
